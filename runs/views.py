        ...
        allruns_cursor = coll.aggregate([
            ...
            {"$group": {"_id": {"source": "$_id.source",
                        ...
                        "data": {"$addToSet": {"host": {"$ifNull": ["$data.data.rse", "$data.data.host"]}, 
                                               ...
                                               "processed_size": {"$ifNull": ["$data.data.size", 0]},
            ...
            {"$group": {"_id": {"source": "$_id.source",
                               ...
                               "host": "$data.host",
                               "count": "$_id.count"},
                        "onsite_rawsize": {"$sum": "$data.raw_size"},
                        "onsite_processed_size": {"$sum": "$data.processed_size"},
                       "onsite_runs": {"$sum": 1}}},
            {"$project": {"_id": "$_id", 
                         "onsite_runs": "$onsite_runs",
                          "onsite_rawsize": "$onsite_rawsize",
                          "onsite_processed_size": "$onsite_processed_size",
